...
def close() -> None:
    ...
    :return:
        - None
    """
    print("Thanks for using Matt's BakkesMod Plugin Updator")
    time.sleep(2)
...
class GUI(object):
    ...
    def __init__(self):
        ...
        self.progress_bar = ttk.Progressbar(root, orient='horizontal', mode='determinate', length=280)
        self.progress_bar.grid(column=0, row=0, columnspan=2, padx=10, pady=20)

        self.value_label = ttk.Label(root, text="Waiting to start")
        self.value_label.grid(column=0, row=1, columnspan=2)

        self.start_button = ttk.Button(root, text='Start', command=lambda: thread.start())
    ...
    def main(self, thread_name: str) -> None:
        ...
        :return:
            - None
        """
        print("\n\t\tWelcome to the Matt's BakkesMod Plugin Updator\n")
        ...
        while not error_occurred:
            if not checkDependencies():
                ...
                break
            root.after_idle(self.update, (100 / 6))
            if not loadSettings():
                error_occurred = True
